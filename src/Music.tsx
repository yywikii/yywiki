  ...
  return (
    <div className="relative h-[700px] mt-4 rounded border border-slate-200 shadow-sm overflow-hidden flex flex-col">
      <div className="bg-white/80 backdrop-blur-md flex flex-col h-full w-full">
        {/* Toolbar */}
        <div className="flex items-center justify-between p-3 border-b border-slate-200 bg-white/50 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="제목, 아티스트 검색..." className="pl-7 pr-7 py-1.5 text-xs border border-slate-200 rounded focus:outline-none focus:border-sky-400 w-56 bg-white/80 transition-all" />
            </div>
            <button onClick={() => { setNewItem(initialItem); setNewTracks([]); setActiveTab('info'); setShowAddModal(true); }} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 text-white text-xs rounded hover:bg-slate-700 transition-colors shadow-sm">
